<?php
//$token = "1716001216:qRNQr9pHB2IJtko4jbEBQEC9DRLELMfz738Yy6HR";
...
$SocialName = filter_input(INPUT_GET, 'SocialName', FILTER_SANITIZE_STRING);
$botName = filter_input(INPUT_GET, 'botName', FILTER_SANITIZE_STRING);

if($SocialName == 'bale'){
    $baseurl = "https://tapi.bale.ai/bot"; 
    $tokentype='baletoken';
    $thinkTXT="دارم فکر می کنم...";
...
function handle_message($message) {
    global $baseurl, $token, $language_code,$instructionsTXT,$firstinstructions,$shortinstructions,$botcredit,$thinkTXT;
    ...
    if($botcredit <= 0) {
        ...
        if ($text === '/start') {
            ...
            $instructionsTXT = $firstinstructions;
                ...
                send_reply_message($chat_id, $thinkTXT);
                checkUsercredit($chat_id, $text);
            } 
...
function askChatGPTWithDB($chat_id, $newQuestion, $pdo) {
    global $ChatGPTapiKey,$instructionsTXT;
...
function checkUsercredit($chat_id, $text) {
